...
class HoloEvTwinInferenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(feat_g, dtype=torch.float32), torch.tensor(feat_l, dtype=torch.float32), torch.tensor(label, dtype=torch.long), f_name
...
def main():
    ...
    model = HoloEvNetClassWiseGated(num_classes=50)
    ...
    with torch.no_grad():
        for x_g, x_l, labels, f_names in tqdm(dataloader):
            ...
            with torch.amp.autocast(device_type=device.type):
                ...
                # クラス別ゲートの計算 (Batch, 50, 2)
                gate_logits = model.gating_layer(feat_global).view(-1, 50, 2)
            ...
            labels = labels.numpy()
            alpha_g = alpha_g.cpu().numpy()
            alpha_l = alpha_l.cpu().numpy()

            for i in range(len(f_names)):
                pred_c = predicted_labels[i]
                is_correct = bool(pred_c == labels[i])
                
                # ★ 予測されたクラスのゲート重みをピンポイントで抽出
                results.append({
                    "ファイル名": f_names[i],
                    "正解ラベル": int(labels[i]),
                    "予測ラベル": int(pred_c),
                    ...
                    "ゲート重み_Global(予測クラス用)": float(alpha_g[i, pred_c]),
                    "ゲート重み_Local(予測クラス用)": float(alpha_l[i, pred_c]),
                })

    json_path = output_dir / "classwise_inference_results.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4, ensure_ascii=False)

    df = pd.DataFrame(results)
    excel_path = output_dir / "classwise_inference_results.xlsx"
