...
import random
from expt_thu_eact_50_chl.config import PROCESSED_DATA_DIR_260707_1
...
PROCESSED_DIR = PROCESSED_DATA_DIR_260707_1

MODEL_SAVE_PATH = CURRENT_DIR / "best_model_augmented.pth"
LOG_FILENAME = CURRENT_DIR / "result_augmented.txt"

# ====================================================
# 0. オンザフライ拡張の再現性を担保するために、乱数シードを固定する関数を定義
# ====================================================
def fix_seed(seed=42):
    # ① Python標準の乱数生成器を固定
    random.seed(seed) 
    
    # ② 行列計算ライブラリ（NumPy）の乱数生成器を固定
    np.random.seed(seed) 
    
    # ③ PyTorch（CPUでの計算用）の乱数生成器を固定
    torch.manual_seed(seed) 
    
    # ④ PyTorch（GPUでの計算用）の乱数生成器を固定
    torch.cuda.manual_seed(seed) 
    
    # ⑤ GPUの計算アルゴリズムの「気まぐれ（非決定性）」を禁止する
    torch.backends.cudnn.deterministic = True 
    
    # ⑥ GPUが自動で計算を「最適化（効率化）」するのを禁止する
