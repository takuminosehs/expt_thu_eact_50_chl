...
# ====================================================
STAGE1_EPOCHS = 50  # グローバルCNN単独育成期間
STAGE2_EPOCHS = 50  # ローカルCNN単独育成期間 (グローバルは完全固定)
STAGE3_EPOCHS = 50  # 両ストリーム完全同期融合期間
# ====================================================
...
class HoloEvNetThreeStageGated(nn.Module):
    def __init__(self, num_classes=50):
        ...
        # ② 新・ローカル・ストリーム (FFT/Gatingを完全排除したピュア幾何形状2D-CNN)
    ...
    def forward(self, x_global, x_local, mode='both'):
        ...
        if mode == 'global_only':
            return y_global, torch.ones((x_global.size(0), 1), device=x_global.device), torch.zeros((x_global.size(0), 1), device=x_global.device)
        ...
        if mode == 'local_only':
            return y_local, torch.zeros((x_global.size(0), 1), device=x_global.device), torch.ones((x_global.size(0), 1), device=x_global.device)
        ...
        return alpha_g * y_global + alpha_l * y_local, alpha_g, alpha_l
...
if __name__ == "__main__":
    ...
    with open(LOG_FILENAME, "w", encoding="utf-8") as f:
        f.write("=== 3-Stage Cascaded Reset Optimization Pipeline ===\n")

        for epoch in range(NUM_EPOCHS):
            # ─── 🔄 【ユーザー指示の完全自動化】3段階コサイン連打数式 ───
            if epoch < STAGE1_EPOCHS:
                ...
                # 指示通り、グローバルを完全ロック(0)し、ローカルとゲートを同期リセット更新
