...
# ② 【本質改善】各ストリーム別の独立初期学習率（LR）設定
# これにより、それぞれの更新幅・スピードを個別にいつでも変更可能です
BASE_LR_GLOBAL = 0.0003  # グローバルCNNの学習スピード
BASE_LR_LOCAL  = 0.0003  # ローカルCNNの学習スピード

# ゲートの更新スピード（0.64で止まった対策として、初期値をバックボーンの10倍に引き上げ）
# もしこれでも到達しなければ、0.005 や 0.010 に上げることでさらに加速できます
BASE_LR_GATING = 0.010
# ====================================================
    ...
    with open(LOG_FILENAME, "w", encoding="utf-8") as f:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if epoch < STAGE1_EPOCHS:
                ...
                optimizer.param_groups[1]["lr"] = BASE_LR_LOCAL * cos_factor
                optimizer.param_groups[2]["lr"] = BASE_LR_GATING * cos_factor  # ゲート専用スピードを適用
                ...
                optimizer.param_groups[0]["lr"] = BASE_LR_GLOBAL * cos_factor
                optimizer.param_groups[1]["lr"] = BASE_LR_LOCAL * cos_factor
                optimizer.param_groups[2]["lr"] = BASE_LR_GATING * cos_factor  # ここも個別に適用
            ...
            # ログ表示に全ストリームの学習率を明示化し、個別のスピード変化を視認できるように拡張
            status = (
