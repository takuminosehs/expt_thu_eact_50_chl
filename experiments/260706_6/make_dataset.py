...
from expt_thu_eact_50_chl.config import HW_DENOISED_DT80_DATA_DIR
...
def preprocess_and_save():
    print(f"🔍 大元データフォルダ（{HW_DENOISED_DT80_DATA_DIR}）内の全データファイルをスキャン中...")
    all_npy_paths = glob.glob(os.path.join(HW_DENOISED_DT80_DATA_DIR, "**", "*.npy"), recursive=True)
    file_map = {os.path.basename(p): p for p in all_npy_paths}
    print(f"識別完了: {len(file_map)} 個のデータファイルを見つけました。")

    for mode in ["train", "test"]:
        txt_file = Path(HW_DENOISED_DT80_DATA_DIR) / f"{mode}.txt"
