...
# config.py からステップ1用の前処理済みデータディレクトリをインポート
from expt_thu_eact_50_chl.config import PROCESSED_DATA_DIR_260710_9
...
# 指定された config.py のデータディレクトリを参照
PROCESSED_DIR = PROCESSED_DATA_DIR_260710_9

# 出力先は指定通り CURRENT_DIR 直下に設定
...
# ====================================================
# 4. HoloEv-Net V4.1 (ステップ1：Avg + Max Pooling 統合版)
# ====================================================
class HoloEvNetBaseV4_SpatialOnly(nn.Module):
    ...
    def __init__(self, num_classes=50):
        ...
        # ─── 【アップグレード】AvgプールとMaxプールの並列化 ───
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.maxpool_final = nn.AdaptiveMaxPool2d((1, 1))
        
        # 特徴量をConcatするため、分類器（Linear）の入力サイズを 512 から 1024 へ変更
    ...
    def forward(self, v_tensor):
        ...
        x = self.layer3(x)
        x = self.spatial3(x)      # 修正：アテンションの戻り値を正しく代入
        ...
        # ─── 【アップグレード】両プール層の適用と結合（Concat） ───
        feat_avg = self.avgpool(x_out).view(x_out.size(0), -1)
        feat_max = self.maxpool_final(x_out).view(x_out.size(0), -1)
        feat = torch.cat([feat_avg, feat_max], dim=1)  # 特徴量次元が1024に拡張される
...
if __name__ == "__main__":
    ...
    with open(LOG_FILENAME, "w", encoding="utf-8") as f:
        f.write("=== THUE-ACT-50 CHL (Avg + Max Pooling Validation) ===\n")
        if params is not None:
            f.write(f"Model Params: {params/1e6:.2f} M (Expanded to 1024ch)\n")
            f.write(f"Model FLOPs: {macs/1e9:.2f} G\n")
        f.write(f"Inference Latency: {latency_ms:.2f} ms\n")
        f.write("=====================================================\n")

        print(f"🚀 ステップ1：Avg + Max Pooling 併用の検証を開始します")
