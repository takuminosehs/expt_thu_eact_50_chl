...
def spatio_temporal_filter(xs, ys, ts_sec, polarities, H=260, W=346, dt_ms=10.0, min_neighbors=2):
    ...
    ※ ts_sec はすでに秒単位にスケーリングされているため、安全に軽量な時間窓に分割できます。
    """
...
def generate_extreme_4ch_local(xs, ys, ts, polarities, H=260, W=346, tau=0.05):
    """
    選択肢1に準拠した4チャネル時空間ボクセルを構築
    Ch1: 全区間Time-Surface
    Ch2: TSのエッジ強度 (空間勾配強度)
    Ch3: ノーマルフロー X成分
    Ch4: ノーマルフロー Y成分
    ...
    # 1. 時空間ノイズ除去を前段に適用 (秒単位のタイムスタンプを使用)
    ...
    # ─── Ch1: Time-Surface (全区間の指数減衰) ───
    # ts_decay: 直近のイベントほど1.0に近く、時間が経つほど0に減衰
    ts_decay = np.exp(-(t_max - t_last) / (tau + 1e-6))
    ts_decay[~has_event] = 0.0  # イベントがない場所を完全に0クリア
    v_tensor_local[0] = ts_decay

    # ─── Ch2: Time-Surfaceの空間勾配強度 (Sobelエッジ) ───
    ...
    # ─── Ch3 & Ch4: ノーマルフロー (Normal Flow) のX成分、Y成分 ───
    # 生の最終タイムスタンプマップから勾配（時間変化率）を計算
...
def main():
    ...
    for mode in ["train", "test"]:
        ...
        print(f"--- Generating {mode.upper()} Splits (Local 4ch with STCF Filter) ---")
        for idx, line in enumerate(tqdm(lines)):
            ...
            # 4チャネルテンソルの生成
            feat_l = generate_extreme_4ch_local(xs, ys, ts, polarities)
