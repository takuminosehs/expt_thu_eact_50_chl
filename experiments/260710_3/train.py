...
# 自分自身の実験ディレクトリ（260709_4）内に作られた前処理データを参照
CURRENT_DIR = Path(__file__).parent.resolve()
PROCESSED_DIR = CURRENT_DIR / "processed_data"

MODEL_SAVE_PATH = CURRENT_DIR / "best_model_normal_flow_spatial.pth"
LOG_FILENAME = CURRENT_DIR / "result_normal_flow_spatial.txt"
...
# ====================================================
# 3. Global Spectral Gating (GSG) モジュール (バグ対策適用済)
# ====================================================
...
# ====================================================
# 4. HoloEv-Net V4.3 (4チャンネル入力・深層空間アテンションモデル)
# ====================================================
class HoloEvNetBaseV4_NormalFlowSpatial(nn.Module):
    def __init__(self, num_classes=50):
        super().__init__()
        resnet = models.resnet18(weights=None)
        
        # 4チャンネル（極性、NormalFlow_y, NormalFlow_x, 位相）
        ...
        # 深い層（手の幾何特徴や携帯の有無が浮かび上がる層）に空間アテンションを限定適用
...
if __name__ == "__main__":
    ...
    model = HoloEvNetBaseV4_NormalFlowSpatial(num_classes=NUM_CLASSES).to(device)
    ...
    with open(LOG_FILENAME, "w", encoding="utf-8") as f:
        f.write("=== THUE-ACT-50 CHL HoloEv-Net-V4.3 (Normal Flow + Spatial Attention) ===\n")
        ...
        print(f"🚀 V4.3 (Normal Flow 差し替え版特訓) を開始します")
