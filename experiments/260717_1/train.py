...
class HoloEvNetClassWiseGated(nn.Module):
    def __init__(self, num_classes=50):
        ...
        # ─── 🛠️ 方針［1-C］による変更点 ───
        # 入力を抽象特徴量(512)から、双方の出力ロジットの結合(num_classes * 2 = 100)に変更
    ...
    def forward(self, x_global, x_local, mode='both'):
        ...
        # ─── 🛠️ 方針［1-C］による変更点 ───
        # 双方のロジット（確信度）を結合してゲートの入力とする。形状: [B, 100]
        y_combined = torch.cat([y_global, y_local], dim=1)
        
        gate_logits = self.gating_layer(y_combined).view(-1, self.num_classes, 2)
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if epoch + 1 == STAGE1_EPOCHS:
                if stage_best_weights is not None:
                    ...
                    msg = f"\n🔄 [Stage-1 Transition] Stage-1の最高精度重み ({stage_best_acc:.2f}%) をロードしました。\n"
                    print(msg)
                    f.write(msg + "\n")
            ...
            # Stage 2 終了時
                ...
                if stage_best_weights is not None:
                    model.load_state_dict(stage_best_weights)
                    msg = f"\n🔄 [Stage-2 Transition] Stage-2の最高精度重み ({stage_best_acc:.2f}%) をロードしました。\n"
                    print(msg)
                    f.write(msg + "\n")
