...
def main():
    ...
    parser = argparse.ArgumentParser(description="各ストリームの予測確率とゲート重みを詳細出力するスクリプト")
    ...
    with torch.no_grad():
        for x_g, x_l, labels, f_names in tqdm(dataloader):
            ...
            with torch.amp.autocast(device_type=device.type):
                ...
                y_global = model.g_classifier(feat_global)  # (Batch, 50)
                ...
                y_local = model.l_classifier(feat_local)  # (Batch, 50)
                ...
                # 5. 分析用にすべて Softmax を適用して「確率値(0.0 ~ 1.0)」に変換
            ...
            for i in range(len(f_names)):
                ...
                # y_global の全クラス確率を展開 (y_global_prob_0 ~ 49)
                for c in range(50):
                    res_dict[f"y_global_prob_{c}"] = float(y_global_probs[i, c])
                
                # y_local の全クラス確率を展開 (y_local_prob_0 ~ 49)
                for c in range(50):
    ...
    # JSON出力
    json_path = output_dir / "detailed_inference_results.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4, ensure_ascii=False)
    print(f"[✓] 詳細JSONを保存: {json_path}")

    # Excel出力 (全サンプルの確率ベクトルが横長に綺麗に並びます)
    df = pd.DataFrame(results)
    excel_path = output_dir / "detailed_inference_results.xlsx"
    df.to_excel(excel_path, index=False, engine="openpyxl")
    print(f"[✓] 詳細Excelを保存: {excel_path}")

    print(f"\n[+] 解析完了！ Accuracy: {df['正誤'].mean()*100:.2f}%")
