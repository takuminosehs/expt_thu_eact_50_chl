...
class ChannelAttention(nn.Module):
    """どのチャンネル（特徴の種類）が重要かを適応的に強調するモジュール"""
...
class SpatialAttention(nn.Module):
    """画像のどの位置（空間・手の付近など）が重要かを適応的に強調するモジュール"""
...
class GlobalSpectralGating(nn.Module):
    def __init__(self, channels, T_prime, H_prime):
        ...
        # --- 🌟 AMP(GradScaler)のエラーを回避するための修正箇所 🌟 ---
        # 複素数型（cfloat）で1つ持つのではなく、実数型（float32）で実部と虚部を別々に定義します
    ...
    def forward(self, x):
        ...
        # 1. 前回の修正：一時的にFloat32にキャスト（cuFFTのサイズ制約対策）
...
if __name__ == "__main__":
    ...
    # 2026年スタンダード: 混合精度(AMP)のためのスケーラー初期化
    scaler = torch.amp.GradScaler()
    
    best_test_acc1 = 0.0

    with open(LOG_FILENAME, "w", encoding="utf-8") as f:
        f.write("=== THUE-ACT-50 CHL HoloEv-Net-V4 (CBAM 統合型) ===\n")
        ...
        print(f"🚀 V4 (CBAMモデル) による「携帯電話 vs 敬礼」対策特訓を開始します")
        ...
        for epoch in range(num_epochs):
            ...
            for features, labels in train_loader:
                ...
                # AMP を用いた省メモリ・高速フォワード
                with torch.amp.autocast(device_type=device.type):
                    outputs = model(features)
                    loss = criterion(outputs, labels)
                
                # スケーラーを介したバックプロパゲーション
