...
# config.py 内の定数と共通ユーティリティ関数のインポート
...
# 自分自身のディレクトリ内にある新しく作った processed_data を参照
CURRENT_DIR = Path(__file__).parent.resolve()
PROCESSED_DIR = CURRENT_DIR / "processed_data"

MODEL_SAVE_PATH = CURRENT_DIR / "best_model_spatial_only_5ch.pth"
LOG_FILENAME = CURRENT_DIR / "result_spatial_only_5ch.txt"
...
# ====================================================
# 4. HoloEv-Net V4.2 (5チャンネル入力・空間アテンション限定モデル)
# ====================================================
class HoloEvNetBaseV4_SpatialOnly5ch(nn.Module):
    def __init__(self, num_classes=50):
        super().__init__()
        resnet = models.resnet18(weights=None)
        
        # 🌟 5チャンネル入力（正極性、負極性、gy, gx, 位相マップ）に対応
...
if __name__ == "__main__":
    ...
    model = HoloEvNetBaseV4_SpatialOnly5ch(num_classes=NUM_CLASSES).to(device)
    ...
    with open(LOG_FILENAME, "w", encoding="utf-8") as f:
        f.write("=== THUE-ACT-50 CHL HoloEv-Net-V4.2 (Spatial Attention 5ch) ===\n")
        ...
        print(f"🚀 V4.2 (5ch分離 + 空間アテンション) による実験を開始します")
